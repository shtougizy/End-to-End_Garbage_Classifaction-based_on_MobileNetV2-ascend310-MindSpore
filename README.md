...
Atlas 200DK 同时充当**开发环境**和**运行环境**，无需额外配置 Ubuntu x86 PC 作为开发机，简化部署流程。
...
5. **开发板联网**（可选）：通过主机网络共享或直接插入网线实现联网
...
通过本次实验，完整体验了 AI 模型从**云端训练**到**边缘部署**的端到端开发流程，对华为昇腾 AI 全栈（MindSpore + CANN + Atlas 硬件）有了系统性认识。
